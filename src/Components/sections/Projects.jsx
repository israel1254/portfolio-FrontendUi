        ...
        <FadeIn delay={0}>
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/30 rounded-full mb-6">
              ...
              <span className="text-sm text-primary font-normal">My Work</span>
            </div>
            <h2 className=" text-4xl lg:text-5xl font-normal text-white mb-4">
              Featured Projects
            </h2>
            <p className='text-lg text-white/60 max-w-2xl mx-auto'>
              Showcasing my bast work and achievements
            </p>
          </div>
        </FadeIn>
          ...
          <div className="relative">
            ...
            </div>
            {
              filteredProjectS.length > 3 && (<>
                ...
                <button onClick={nextSlide} disabled={currentIndex >= filteredProjectS} className='flex absolute right-0 top-1/2 -translate-y-1/2 -translate-x-2 lg:-translate-x-4 items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full hover:bg-white/20 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed z-10' aria-label='Previous projects'>
                ...
                <div className="flex items-center justify-center gap-2 mt-8">
                  {
                    Array.from({ length: Math.max(0, filteredProjectS.length - 2) }).map((_, index) => (
                      <button key={index} onClick={() => scrollToIndex(index)} className={`transition-all duration-300 rounded-full ${index === currentIndex ? 'bg-primary w-6 h-2' : 'bg-white/30 w-2 h-2 hover:bg-white/50'}`} aria-label={`Go to slide ${index + 1}`}></button>
                    ))
                  }
                </div>
              )}
