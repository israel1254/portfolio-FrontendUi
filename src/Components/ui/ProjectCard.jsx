    ...
    <div className='group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300'>
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 flex items-center gap-3">
          {
            githubUrl && (<a href={githubUrl} target='_blank' rel='noopener noreferrer' className='p-2.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-primary/50 transition-all duration-300 hover:scale-110' title='view Code'>
              <Github className='w-4 h-4 text-white' />
            </a>
            )}
          {
            demoUrl && (<a href={demoUrl} target='_blank' rel='noopener noreferrer' className='p-2.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:border-primary/50 transition-all duration-300 hover:scale-110' title='view Demo'>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {
            technologies.map((tech, index) => (<span key={index} className='px-3 py-1 text-xs font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-colors duration-200'>
              {tech}
            </span>
            ))
          }
        </div>
        {metrics && (<div className="flex items-center gap-2 pt-3 border-t border-white/10">
