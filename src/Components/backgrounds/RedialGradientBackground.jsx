...
const RedialGradientBackground = ({ variant = 'hero', gradients = [] }) => {
  ...
  const generateGradient = (colors) => {
    ...
    return `radial-gradient(circle at center, transparent 30%, ${colorStops}, transparent 60%, transparent 100%)`;
  }
  return (
    <div className='absolute inset-0 overflow-hidden pointer-events-none'>
      {
        activeGradients.map((gradient, index) => (
          <div key={index} className={`absolute ${gradient.position} ${gradient.size} rounded-full`} style={{ 
